...
class IconTagTest(SimpleTestCase):
    ...
    def test_svg_renders_with_custom_defaults(self):
        with self.settings(account_management_DEFAULTS={"height": "2em", "fill": "red"}):
            svg = icon("home")
    ...
    def test_alternative_directory_setting(self):
        with self.settings(account_management_ICONS_DIR="alt_dir"):
            svg = icon("alt_dir")
            self.assertIn('height="1em"', svg)
    ...
    def test_when_given_invalid_file_it_should_fail_silently(self):
        svg = icon("home")
        template = Template("{% load account_management %}{% icon 'home' %}")
